...
const Input = (args) => {
  ...
  const intermediateStage =
    multiValidation?.length > 0 && isErrors && inputProps?.errors?.length < multiValidation?.length
  ...
  return (
    <div className={"input-wrapper"}>
      <label className={"input-label"}>
        <div>{label}</div>
        <input type="text" placeholder={placeholder} className={inputClassName} {...inputProps} {...props} />
        {multiValidation?.length > 0 ? (
          <ul className="validation-list text-s">
            {multiValidation.map((item, index) => (
              <li
                className={`validation-list__item ${
                  (isErrors && inputProps?.errors.find((errorName) => errorName === item)) || !inputProps?.value
      ...
      <div className={"input-alter-action"}>{alterAction}</div>
    </div>
  )
}
