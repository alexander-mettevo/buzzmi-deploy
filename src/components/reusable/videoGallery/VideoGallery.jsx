...
const VideoGallery = ({ src, title }) => {
  const [showVideoPlayer, setShowVideoPlayer] = useState(false)
  const [duration, setDuration] = useState(null)
  const previewVideoRef = useRef(null)
  const getDuration = () => {
    const formatDuration = moment
      .utc(moment.duration(previewVideoRef.current.duration, "seconds").asMilliseconds())
      .format("mm:ss")
      ...
      <LightBox isOpened={showVideoPlayer} onClose={() => setShowVideoPlayer(false)}>
        <div className="video-gallery__player-body">
          <div className="video-gallery__player-holder">
            <VideoPlayer url={src} title={title} tracks={tracks} getDuration={(val) => console.log(val)} />
          </div>
        </div>
      </LightBox>
    </div>
