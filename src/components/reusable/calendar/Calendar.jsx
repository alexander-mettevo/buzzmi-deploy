...
const Calendar = ({
    ...
    useEffect(() => {
        ...
        if(moment().dayOfYear(Number(firstDayOfCurrentWeek + 6)).get("date") !== 7) {
            triggerClickOnPrev();
        }
    ...
    const getLabelText = ({direction}) => {
        if (direction === "down") {
            return (
                <span onClick={() => isForWeek ? onChangeWeek(-(dayIncrement + 1)) : null}
    ...
    const minAndMaxDate = {
        ...
        "max": new Date(moment(value).dayOfYear(Number(firstDayOfCurrentWeek) + dayIncrement).format(dateFormat))
    }
    ...
    return (
        <div className={"calendar" + (noNavigation ? " calendar--no-navigation" : "")}>
            ...
            <div className="calendar__body">
                {!redrew && <CalendarComponent defaultActiveStartDate={new Date(value)}
                                   value={value}
            ...
            {buttonProps && <div className="calendar__btn-wrapper text-center">
                <button className="calendar__btn" onClick={() => buttonProps.isDateAnchor ? onReset() : buttonProps.handler()}>{buttonProps.text}</button>
            </div>}
            {hasFooter && <div className="calendar__footer d-flex">
